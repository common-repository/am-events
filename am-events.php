...
/*
  ...
  Plugin URI: https://wordpress.org/plugins/am-events/
...
add_action('admin_enqueue_scripts', 'am_custom_script_post');
...
function am_custom_script_post($hook) {
	global $post;
	...
	if ( !is_admin() || $post->post_type !== 'am_event') {
		return;
	}

	// Add Timepicker script
	wp_register_script('flatpickr', plugins_url('/script/flatpickr.min.js', __FILE__));
	wp_enqueue_script('flatpickr', plugins_url('/script/flatpickr.min.js', __FILE__));

	// Custom script for assigning jquery to inputs
	wp_register_script('am_custom_script', plugins_url('/script/admin-post.js', __FILE__));
	...
	$startDate = mysql_to_rfc3339(get_post_meta($post->ID, 'am_startdate', true));
	$endDate = mysql_to_rfc3339(get_post_meta($post->ID, 'am_enddate', true));
	...
	wp_localize_script('am_custom_script', 'localization', $localization);
	wp_localize_script('am_custom_script', 'event_data', $eventData);
	wp_enqueue_script('am_custom_script', plugins_url('/script/admin-post.js', __FILE__, null, true));
...
function am_custom_css() {
	...
	wp_enqueue_style('flatpickr', plugins_url('/css/flatpickr.min.css', __FILE__));
...
function am_meta_box_content($post) {
	...
	<table>
		<tr>
			...
			<td>
				<input type="text" id="am_startdate" name="am_startdate"/>
			</td>
		</tr>
		<tr>
			<td align="right">
			...
			<td>
				<input type="text" id="am_enddate" name="am_enddate"/>
			</td>
		</tr>
	</table>
